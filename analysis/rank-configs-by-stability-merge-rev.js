...
var main = async (argv) => {
  ...
  dataFiles.forEach(({file, data}) => {
    // Find name from file name
    var name = file.split('/').slice(0, -1).join('/');
    var rev  = file.split('/').slice(-1)[0];
    ...
    if (!confs[name]) {
      confs[name] = {
        name:   name,
        data:   {},
        tests:  {}, // Mapping from: rev+suiteName      -> result object
        cases:  {}  // Mapping from: rev+suiteName+case -> array of results
      }
    }
    var conf = confs[name];

    // Check that rev isn't set yet
    if (conf.data[rev]) {
      throw new Error(name + " has rev twice: " + rev);
    }
    conf.data[rev] = data;

    // For each test under consideration (from TESTS environment variable)
    for (var suiteName of testNames) {
      conf.tests[rev + ':' + suiteName] = _.find(data, r => {
        return r.testrun.suite === suiteName;
      });
      assert(conf.tests[rev + ':' + suiteName],
             "Missing test: " + rev + ':' + suiteName + " for " + file);
      // We call all entries in the test suite for cases, and name them as
      // suite + case, to build a dictionary of them:
      for (var caseName of _.keys(conf.tests[rev + ':' + suiteName].results)) {
        var cN = rev + ':' + suiteName + ':' + caseName;
        conf.cases[cN] = conf.tests[rev + ':' + suiteName].results[caseName];
      }
    }
  ...
  var configurations = _.values(confs);
